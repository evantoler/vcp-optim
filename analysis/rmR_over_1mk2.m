...
version = 'log';    % 'log' to see behavior near boundary
...
% determine the limiting value of (r - R) / (1 - k^2) * [complicated]
val = (rr - R)./(1-k2);
val = val .* (ellipticE(omega, k2) - k2.*sin(omega)*cos(omega)./sqrt(1-k2.*sin(omega).^2));

figure;
switch version
    case 'log'
        loglog(t-t0, abs(val)); hold on
        loglog(t-t0, (t-t0).^(1), 'k--') % product of (r-R)/(1-k2) with extra term -> 0 at first order
    otherwise
        plot(t, val + flip(val))
end
